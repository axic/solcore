...
instance Resolve S.Exp where
  ...
  -- `type(C).publicMethods` (and its sugar `type(C)`) is a compiler primitive:
  -- it desugars to a call to a per-contract helper function generated by the
  -- PublicMethods desugarer (see Solcore.Desugarer.PublicMethods).  The helper
  -- yields the contract's public methods as a `Proxy` over a tuple of
  -- `Method(...)` types, from which the interface id is computed.
