...
instance Resolve S.Exp where
  ...
  -- `type(C).publicMethods()` is a compiler primitive: it desugars to a call
  -- to a per-contract helper function generated by the PublicMethods desugarer
  -- (see Solcore.Desugarer.PublicMethods).  The helper is a function that
  -- allocates and builds a fresh array of public-method selectors on each call,
  -- used to compute the contract's interface id.
